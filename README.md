...
- 把 Brief、用户原始描述、业务背景和限制条件整理成标准 PRD 草稿。
- 先补齐 PRD 的基本骨架：问题 / 目标、用户 / 角色、范围、非范围、需求、用户流程、成功场景、失败场景、边界场景、开放问题。
- 把不确定内容标为假设或待确认问题，不把它们直接写成已确认规则。

为什么做：

- 需求早期经常是碎片化描述，直接写 BDD 容易遗漏“为什么做、给谁做、做什么、不做什么”。
- PRD 草稿先把产品语义整理清楚，再交给 ClearFlow 的确认流程和 BDD 转换，可以减少 Agent 自行脑补业务规则。
- 它把“文档写作”从“行为验收”里拆出来：PRD 负责说明产品意图，BDD 负责验证已确认行为。

有什么效果：

- 使用者能先审阅一份更完整的 PRD 初稿，而不是直接面对零散问题。
- BDD 场景会来自已确认的 PRD 行为，后续 Test Strategy 更容易追溯到真实需求。
- Plan / Task Handoff 会获得更清晰的范围、非范围和开放问题，降低实现阶段误做、漏做或扩大范围的风险。

### 为什么做

自然语言需求经常含糊。BDD 让行为变得可测试、可审查，也让 Agent 在实现前有明确目标。

PRD 负责说明“为什么做、给谁做、做什么、不做什么”。BDD 负责把已确认的 PRD 行为转换成可验收、可测试的场景。两者不冲突；PRD-Writer 只能帮助写 PRD 初稿，不能跳过 BDD。
